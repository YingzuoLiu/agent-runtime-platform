...
from .sandbox import ToolExecutionStatus, ToolRegistry, ToolSandbox
...
class DynamicToolLoop:
            ...
            if step.status == ToolCallStatus.COMPLETED:
                ...
                if not isinstance(decision, CallToolDecision):
                    self._fail(
                        context.run_id,
                        "invalid_planner_decision",
                        "Completed tool step is not backed by a CALL_TOOL decision.",
                    )
                spec = self.tool_registry.resolve(decision.tool_name)
                if spec is None:
                    self._fail(
                        context.run_id,
                        "unknown_tool",
                        "A completed tool is no longer registered.",
                    )
                try:
                    ...
                    self._fail(
                        context.run_id,
                        "invalid_tool_arguments",
                        "Persisted tool arguments no longer pass schema validation.",
                    )
                if step.tool_name != decision.tool_name or step.input_hash != self._stable_hash(normalized):
                    self._fail(
                        context.run_id,
                        "invalid_planner_decision",
                        "Completed tool evidence does not match its planner decision.",
                    )
    ...
    def _decision_payload_for_step(self, run_id: str, step_id: str) -> dict[str, Any]:
        for event in self.workflow_store.list_events(run_id):
        ...
        self._fail(
            run_id,
            "invalid_planner_decision",
            f"Tool step {step_id} has no durable planner decision.",
        )
