...
This suite turns the durable execution plane's storage guarantees into backend-parameterized,
executable scenarios. It proves semantic compatibility through observable outcomes, not through
...
A future backend is not interchangeable merely because its method signatures resemble SQLite. It
must execute the same scenarios through a new test-side adapter and produce the same typed outcomes,
durable state, event order, revisions, attempts, and sanitized error codes.
...
Every test ID below is parameterized by backend. The only current parameter is `[sqlite]`.
...
| **I4 Different Threads remain independently schedulable** | Two queued Runs have different Thread identities. | Two stores claim concurrently through the same barrier scenario as I3. | Both Run IDs are claimed; one Thread does not serialize unrelated work. | `tests/conformance/test_run_store_contract.py::test_i4_different_threads_remain_independently_schedulable[sqlite]` |
...
| --- | --- | --- |
| Execution/step identity, append-only ordering, cursor reads, restart visibility | Typed `CREATED`, `EXISTING`, input/workflow mismatch outcomes; sequences are contiguous; reopened store returns the same terminal execution, step, and events. | `test_workflow_identity_event_order_and_restart_visibility[sqlite]` |
...
1. add a test-side adapter that creates and recreates the backend and supplies equivalent clock and
   fault-injection hooks;
2. add it to the conformance fixture parameter list;
3. run the same scenario IDs unchanged;
...
It extracts the existing SQLite single-host semantics into reusable executable contracts.
