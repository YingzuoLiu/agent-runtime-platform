resource "aws_secretsmanager_secret" "runtime_postgres_dsn" {
  ...
  recovery_window_in_days  = var.runtime_secret_recovery_window_days == 0 ? null : var.runtime_secret_recovery_window_days
  force_delete_without_recovery = var.runtime_secret_recovery_window_days == 0
}
